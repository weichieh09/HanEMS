<template>
  <div>
    <h2 id="page-heading" data-cy="EqItemHeading">
      <span v-text="$t('hanEmsApp.eqItem.home.title')" id="eq-item-heading">Eq Items</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hanEmsApp.eqItem.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EqItemCreate' }" custom v-slot="{ navigate }">
          <button
            @click="navigate"
            id="jh-create-entity"
          ...
          >
            ...
            <span v-text="$t('hanEmsApp.eqItem.home.createLabel')"> Create a new Eq Item </span>
          </button>
        </router-link>
      </div>
    </h2>
      ...
      <table class="table table-striped" aria-describedby="eqItems">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('hanEmsApp.eqItem.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('hanEmsApp.eqItem.description')">Description</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('createDate')">
              <span v-text="$t('hanEmsApp.eqItem.createDate')">Create Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('modifyDate')">
              <span v-text="$t('hanEmsApp.eqItem.modifyDate')">Modify Date</span>
        ...
        <tbody>
          <tr v-for="eqItem in eqItems" :key="eqItem.id" data-cy="entityTable">
            <td>
              <router-link :to="{ name: 'EqItemView', params: { eqItemId: eqItem.id } }">{{ eqItem.id }}</router-link>
            </td>
            <td>{{ eqItem.name }}</td>
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link :to="{ name: 'EqItemView', params: { eqItemId: eqItem.id } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm details" data-cy="entityDetailsButton">
                    <font-awesome-icon icon="eye"></font-awesome-icon>
                    <span class="d-none d-md-inline" v-text="$t('entity.action.view')">View</span>
                  </button>
                </router-link>
                <router-link :to="{ name: 'EqItemEdit', params: { eqItemId: eqItem.id } }" custom v-slot="{ navigate }">
...
<script lang="ts" src="./eq-item.component.ts"></script>
