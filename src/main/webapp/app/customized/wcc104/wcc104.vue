      ...
      <table class="table table-striped" aria-describedby="equipment">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('totalEle')">
              <span v-text="$t('hanEmsApp.equipment.totalEle')">Total Ele</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'totalEle'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('rentedEle')">
              <span v-text="$t('hanEmsApp.equipment.rentedEle')">Rented Ele</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'rentedEle'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('currentEle')">
              <span v-text="$t('hanEmsApp.equipment.currentEle')">Current Ele</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'currentEle'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="equipment in equipment" :key="equipment.id" data-cy="entityTable">
            ...
            <td>{{ equipment.totalEle }}</td>
            <td>{{ equipment.rentedEle }}</td>
            <td>{{ equipment.currentEle }}</td>
            <td class="text-right">
              <div class="btn-group">
...
<script lang="ts" src="./wcc104.component.ts"></script>
